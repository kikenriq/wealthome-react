...
const Header = () => {
  ...
  return (
    <header className={`${isActive ? 'bg-primary-40 h-[72px]' : 'bg-none h-[100px]'}  fixed top-0 left-0 w-screen py-6 px-5 z-30 flex justify-between items-center`}>
        ...
        <button className='bg-white cursor-pointer inline-flex -rounded--radius-small text-[15px] p-2'
        onClick={() => setMobileNav(!mobileNav)}>
          <span class="material-symbols-outlined">menu</span>
        </button>

        <div className={`${mobileNav ? ' opacity-100 scale-100' : ' opacity-0 scale-50'} transition-all origin-top-right absolute text-[15px] top-[75%] z-40 right-[16px] shadow-md bg-neutral-100 min-w-[260px] px-8 py-8 -rounded--radius-large`}>
          ...
          <button className='px-8 py-5 bg-primary-100 text-neutral-100 -rounded--radius-medium'>
